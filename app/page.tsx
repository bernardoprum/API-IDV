...
import { XCircle, Upload, RotateCcw, Loader2, Key, Eye, EyeOff, Settings, Zap } from "lucide-react"
...
export default function VerificationPortal() {
  ...
  const generateDemoHMAC = async (payload: string, secretKey: string) => {
    if (!secretKey) return "demo_signature_" + Math.random().toString(36).substring(7)
  ...
  const handleDemoSubmit = async () => {
    ...
    try {
      ...
      const sessionId = "demo-session-" + Math.random().toString(36).substring(2, 15)
      ...
      if (apiKeys.apiKey && apiKeys.privateKey) {
        ...
        const selfieHMAC = await generateDemoHMAC(sessionId, apiKeys.privateKey)

        console.log("✅ HMAC for DOCUMENT_FRONT:", frontHMAC.substring(0, 16) + "...")
        console.log("✅ HMAC for DOCUMENT_BACK:", backHMAC.substring(0, 16) + "...")
        console.log("✅ HMAC for FACE:", selfieHMAC.substring(0, 16) + "...")

        console.log("📡 Demo API Headers:")
        console.log("   X-AUTH-CLIENT:", apiKeys.apiKey.substring(0, 8) + "...")
      ...
      console.log(`   Front: ${files.frontLicense!.name} (${files.frontLicense!.size} bytes)`)
      console.log(`   Back: ${files.backLicense!.name} (${files.backLicense!.size} bytes)`)
      console.log(`   Selfie: ${files.selfie!.name} (${files.selfie!.size} bytes)`)
      ...
      const mockResults: VerificationData = {
        ...
        documentValidation: {
          ...
          documentNumber: "DL" + Math.random().toString().substring(2, 11),
          isValid: decision === "approved",
          confidence: decision === "approved" ? 85 + Math.random() * 15 : 40 + Math.random() * 30,
        },
        faceMatch: {
          confidence: decision === "approved" ? 80 + Math.random() * 15 : 30 + Math.random() * 40,
  ...
  const handleSubmit = mode === "demo" ? handleDemoSubmit : handleProductionSubmit
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 p-4">
      <div className="max-w-4xl mx-auto">
        <div className="text-center mb-8">
          <h1 className="text-4xl font-bold text-gray-900 mb-2">Identity Verification Portal</h1>
          <p className="text-lg text-gray-600">Secure document verification powered by Veriff</p>
            ...
            <div className="bg-white rounded-lg p-1 border border-gray-200">
              <button
                ...
                className={`px-4 py-2 rounded-md text-sm font-medium transition-colors flex items-center gap-2 ${
                  mode === "demo" ? "bg-blue-100 text-blue-700" : "text-gray-500 hover:text-gray-700"
                }`}
              >
              ...
              <button
                ...
                className={`px-4 py-2 rounded-md text-sm font-medium transition-colors flex items-center gap-2 ${
                  mode === "production" ? "bg-green-100 text-green-700" : "text-gray-500 hover:text-gray-700"
                }`}
              >
          ...
          <div className="mt-4 text-xs text-gray-500 bg-white rounded-lg p-3 border">
            {mode === "demo" ? (
              <>
                <strong>🎯 Demo Mode:</strong> Upload any images to see the verification flow. Optionally add your
                Veriff API keys to test real authentication logic.
                <br />
                <strong>📝 Ready for Testing:</strong> Check console logs to see exactly how real Veriff integration
                works.
              </>
            ) : (
              <>
                <strong>🚀 Production Mode:</strong> Uses real Veriff API with your configured environment variables.
                <br />
                <strong>⚡ Live Integration:</strong> Actual verification results from Veriff servers.
              </>
            )}
          </div>
        </div>
          ...
          <Card className="mb-6 bg-blue-50 border-blue-200">
            ...
            {showApiKeys && (
              <CardContent className="space-y-3">
                <div>
                  <label className="block text-xs font-medium text-blue-700 mb-1">API Key (X-AUTH-CLIENT)</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-xs font-medium text-blue-700 mb-1">Private Key (Shared Secret)</label>
                  <div className="relative">
                    <input
                      type={apiKeys.showPrivateKey ? "text" : "password"}
                      value={apiKeys.privateKey}
                      onChange={(e) => setApiKeys((prev) => ({ ...prev, privateKey: e.target.value }))}
                      className="w-full text-xs border border-blue-300 rounded px-2 py-1 pr-8"
                      placeholder="Your Veriff private key"
                    />
                    <button
                      type="button"
                      onClick={() => setApiKeys((prev) => ({ ...prev, showPrivateKey: !prev.showPrivateKey }))}
        ...
        <Card className="mb-6">
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              ...
              Document Upload
              <Badge variant={mode === "demo" ? "secondary" : "default"}>{mode === "demo" ? "Demo" : "Live"}</Badge>
            </CardTitle>
            <CardDescription>
              Please upload clear photos of your driver's license (front and back) and a selfie photo
            </CardDescription>
          </CardHeader>
            ...
            {isSubmitting && (
              <div className="space-y-2">
                <div className="flex justify-between text-sm text-gray-600">
                  <span>{mode === "demo" ? "Processing demo..." : "Processing verification..."}</span>
                  <span>{progress}%</span>
                </div>
                <Progress value={progress} className="w-full" />
                {mode === "demo" && (
                  <p className="text-xs text-blue-600 text-center">Check browser console for detailed logs</p>
                )}
              </div>
            )}
              ...
              <Button onClick={handleSubmit} disabled={!isFormValid || isSubmitting} className="flex-1" size="lg">
                {isSubmitting ? (
                  ...
                  <>
                    ...
                    {mode === "demo" ? "Start Demo Verification" : "Start Verification"}
                  </>
                )}
              </Button>
              {(result || error) && (
